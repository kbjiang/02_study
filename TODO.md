## Courses
1. Cornell, cs6783 Machine Learning Theory
2. Stanford CS229M Machine Learning Theory
	1. [Stanford CS229M: Machine Learning Theory - Fall 2021 - YouTube](https://www.youtube.com/playlist?list=PLoROMvodv4rP8nAmISxFINlGKSK4rbLKh)
3. Mathematics of Neural networks
	1. https://arxiv.org/pdf/2403.04807
4. MIT matrix calculus
	1. https://ocw.mit.edu/courses/18-s096-matrix-calculus-for-machine-learning-and-beyond-january-iap-2023/
5. Convex optimization Stephen Boyd ([https://web.stanford.edu/class/ee364a/lectures.html](https://web.stanford.edu/class/ee364a/lectures.html))
6.  Physics informed ML

### Videos
1. [Stanford CS230: Deep Learning I Autumn 2025](https://youtube.com/playlist?list=PLoROMvodv4rNRRGdS0rBbXOUGA0wjdh1X&si=3fdOsE71YITQgMGm)
2. [Stanford CME296 Diffusion & Large Vision Models | Spring 2026 | Lecture 1 - Diffusion](https://www.youtube.com/@stanfordonline)
3. https://x.com/ZitongYang0/status/2029034067992437139 "Today I defended my Ph.D. thesis at Stanford: Continually self-improving AI."
## Book
1. LM interview handbook https://www.linkedin.com/feed/update/urn:li:activity:7446817744513028097/
2. Foundations of Large Language Models https://arxiv.org/abs/2501.09223
3. syntax: a cognitive approach
4. GPT图解大模型是怎样构建的 https://drive.google.com/file/d/1IlWY2z8USBm3TKBUfLNDcTykltdiJriA/view?usp=drive_link
## Blogs
1. Frontier model training methodologies Alex Wa https://djdumpling.github.io/2026/01/31/frontier_training.html
2. Eugene Yan https://eugeneyan.com/

### MISC
1. https://www.llm-visualized.com/?token=4&generation=0&kvCache=0


## ???
1. why non-linearity is needed in NN
	1. linear transformation can only do 1:1 mapping; need non-linearity to do more complicated transformation. E.g., all images of cats and dogs ends up being two points in the output layer, n:1 mapping.
2. There are two circles in number 8; MLP needs to learn both, CNN only need to learn once.
3. Latent space and translation: encode Chinese to latent space, then decode latent embedding to English
4. tokens in LLM is like pixel in CNN; embedding dim is like channel (RGB)
5. In encode-decode, cross attention, `q` from decoder plays the role of input, `k` and `v` are from encoder
